...
> - [Introduced support for `alias` as service container name](https://gitlab.com/gitlab-org/gitlab/-/issues/421131) in GitLab Runner 16.7.
...
With GitLab Runner v16.7 and later, the `service alias` can be used as the service container name if the alias provided
meet the `RFC 1123 definition of a DNS label`:

- The alias length is no longer than 253 characters.
- The alias is made of `only alphanumeric` and `-` characters.
- The alias should not end by `-`

When at least one of these conditions is not met, the service container name falls back to the `svc-i` naming convention.
